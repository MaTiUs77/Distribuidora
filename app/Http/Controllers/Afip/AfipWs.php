 ...
 * AFIP Web Services
 *
 * @link http://www.afip.gob.ar/fe/documentos/manual_desarrollador_COMPG_v2_10.pdf WSFE Documentacion
 *
 * @author 	Ivan Muñoz ivanalemunioz@gmail.com
 * @package Afip
 * @version 0.1
 **/
class AfipWs extends Controller
{
    /**
...
//        $dir_name = dirname(__FILE__);
     ...
     *
     * @return AfipWsTa Token Autorization for AFIP Web Service
     **/
    private function GetServiceTA($service, $continue = TRUE)
    {
        if (file_exists($this->RES_FOLDER.'TA-'.$service.'.xml')) {
            ...
            if ($actual_time < $expiration_time)
                return new AfipWsTa($TA->credentials->token, $TA->credentials->sign);
...
class AfipWsTa extends  Controller{
    /**
     * WSAA Token
     *
     * @var string
